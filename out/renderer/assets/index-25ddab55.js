...
class FlipNumber {
  ...
  getTimingNums() {
    ...
    this.nums = (hourString + "" + minuteString + secondString).substring(6 - len).split("").map((n) => +n);
  }
  //获取时间的数字
  getClockNums() {
    this.nums = dayjs().format("HHmmss").split("").map((n) => +n);
  }
  //定时器数字
...
class FlipClock extends FlipNumber {
  ...
  updateDivNumber() {
    this.divs.forEach((divs, index) => {
      ...
      div.addEventListener("animationend", () => {
        divs.forEach((div2) => {
          div2.dataset.before = String(before);
          div2.dataset.after = String(after);
        });
  ...
  getDivs() {
    this.divs = Array.from(this.main.querySelectorAll("section")).map(
